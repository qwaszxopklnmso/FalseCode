...
- Declarations: `x -> T;`, `x = v -> T;`, `x[] -> T`→`T x[]` (C-style, empty bracket kept verbatim; `c[] = "hi" -> char`→`char c[] = {"hi"};`), `x<> -> T`→`vector<T>` (nested `<>` pairs → nested vectors; `<<`/`>>` lex as shift ops and are re-split in the declarator), `x[N] -> T`→`T x[N]`, `x[N] = v -> T`→`T x[N] = {v};` (whole-array init), `x[N][M] -> T`→`T x[N][M]`. A type annotation may carry a C++-style array suffix: `arr = {1,2,3} -> int[3];` → `int arr[3] = {1,2,3};`. Anything without `->` is a plain assignment (no implicit declaration).
